...
class TestNow:
    def test_now_is_in_correct_timezone(self) -> None:
        now = localtime.now()
        assert str(now.tzinfo) == settings.TIME_ZONE


class TestSecondsInTheFuture:
    def test_seconds_in_future(self) -> None:
        with time_machine.travel("2020-01-01 12:00:00.000", tick=False):
            assert localtime.seconds_in_the_future(
                1
            ) == localtime.as_localtime(
                datetime.datetime(2020, 1, 1, 12, 0, 1, tzinfo=localtime.UTC)
            )
            assert localtime.seconds_in_the_future(
                # This is deliberately checking that an invalid call still
                # gives a ¯result.
...
class TestSecondsInThePast:
    def test_seconds_in_past(self) -> None:
        with time_machine.travel("2020-01-01 12:00:00.000", tick=False):
            assert localtime.seconds_in_the_past(1) == localtime.as_localtime(
                datetime.datetime(2020, 1, 1, 11, 59, 59, tzinfo=localtime.UTC)
            )
            assert localtime.seconds_in_the_past(
                # This is deliberately checking that an invalid call still
                # gives a result.
                1.5  # type: ignore[arg-type]
            ) == localtime.as_localtime(
...
class TestDate:
    def test_date_calculation_near_midnight_during_bst(self) -> None:
    ...
    def test_date_calculation_near_midnight_outside_of_bst(self) -> None:
        near_midnight_in_utc = datetime.datetime(
            2016, 11, 1, 23, 50, 0, tzinfo=localtime.UTC
        )
        assert (
            localtime.date(near_midnight_in_utc) == near_midnight_in_utc.date()
        )

    @pytest.mark.parametrize(
        "tz", (zoneinfo.ZoneInfo("Etc/GMT-10"), localtime.UTC)
    )
    def test_date_calculation_specifying_other_timezone(
        self, tz: zoneinfo.ZoneInfo
    ) -> None:
        near_midnight = datetime.datetime(2016, 6, 1, 23, 50, 0, tzinfo=tz)
        assert localtime.date(near_midnight, tz=tz) == (near_midnight.date())

    def test_datetime_not_supplied(self) -> None:
        """
        Check that we do not fallback to today if a datetime is not passed to
        the function - we have localtime.today for that.
        """
        with pytest.raises(
            TypeError, match="You must supply a datetime to localtime.date"
        ):
            # A deliberate invalid call (so we ignore the type-checking error).
            localtime.date(None)  # type: ignore[arg-type]
...
class TestDateOfDayBefore:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                factories.local.dt("2019-06-01 00:00:00"),
                factories.date("2019-05-31"),
            ),
            # Note that this next example is *the same time*, but expressed in
            # UTC. So again, it correctly returns the same date. But you might
            # not be expecting it when you see it.
            (
                factories.utc.dt("2019-05-31 23:00:00"),
                factories.date("2019-05-31"),
            ),
            # A couple of 'normal' examples
            (
                factories.local.dt("2019-01-23 05:30:00"),
                factories.date("2019-01-22"),
            ),
            (
                factories.utc.dt("2019-01-23 05:30:00"),
                factories.date("2019-01-22"),
            ),
        ],
    )
    def test_returns_expected_results(
        self, dt: datetime.datetime, date_of_day_before: datetime.date
    ) -> None:
        assert localtime.date_of_day_before(dt) == date_of_day_before


class TestMidnight:
    def test_midnight_calculation_for_bst_date(self) -> None:
    ...
    def test_midnight_calculation_for_utc_dt(self) -> None:
        dt = factories.utc.dt("2017-12-20T16:00")

        assert localtime.midnight(dt) == localtime.datetime(2017, 12, 20, 0, 0)

    def test_midnight_calculation_without_date_uses_today(self) -> None:
        midnight = localtime.midnight()
        assert midnight.date() == localtime.today()
        assert midnight.tzinfo is not None
        assert midnight.hour == 0

    def test_convert_date_to_midnight_and_back(self) -> None:
        date = datetime.date(2016, 7, 2)
        midnight = localtime.midnight(date)
        midnight_utc = midnight.astimezone(localtime.UTC)
        assert localtime.date(midnight_utc) == date

    def test_midnight_in_different_timezone(self) -> None:
    ...
    def test_doesnt_change_date_of_already_midnight_datetime(self) -> None:
        """
        Check that if we pass a datetime to `midnight()` which is *already*
        midnight in the relevant timezone, it doesn't get changed.
        """
        midnight = datetime.datetime(
            2020, 6, 1, 23, tzinfo=datetime.timezone.utc
        )
    ...
    def test_localtime_midnight_calculation_for_naive_datetime_and_no_timezone(
        ...
        expected_midnight: datetime.datetime,
    ) -> None:
        """
        Test the behavior of localtime.midnight() when passed a naive datetime
        and NOT passed a tz

        We prefer to not pass a naive datetime to midnight(), but if you do,
        this is the behaviour with no tz argument: 1. the naive datetime is
        assumed to be in the current Django timezone 2. get the date of that
        datetime 3. return midnight at the start of that date, in the current
        Django timezone.
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                # then converted to UTC - moved 10 hours back - changes to
                # previous date
                "UTC",
                # midnight returned is previous date - this result can be
                # unexpected
            ...
            (
                ...
                # then converted to Pacific/Auckland - moved 2 hours ahead -
                # changes to next date
                "Pacific/Auckland",
                # midnight returned is next date - this result can be
                # unexpected
                datetime.datetime(2021, 6, 25, 0, 0, 0),
            ),
            (
                # This example was the cause of a bug in Australia: this is
                # assumed to be in the current django timezone (Sydney), which
                # on this date is UTC+11 (DST)
            ...
            (
                # this is assumed to be in the current django timezone
                # (Sydney), which on this date is UTC+10
    ...
    def test_localtime_midnight_calculation_for_naive_dt_specified_timezone(
        self,
        naive_datetime: datetime.datetime,
        specified_timezone: str,
        expected_midnight: datetime.datetime,
    ) -> None:
        """
        Test the behavior of localtime.midnight() when passed a naive datetime
        ...
        We prefer to not pass a naive datetime to midnight(), but if you do,
        this is the behaviour when tz argument is specified: 1. the naive
        datetime is assumed to be in the current Django timezone
            (NOT the passed timezone)
        2. convert that moment in time to the specified timezone
        3. get the date of that datetime
        3. return midnight at the start of that date, in the specified
           timezone.
        """
        ...
        actual_midnight = localtime.midnight(
            naive_datetime, tz=specified_timezone_obj
        )
...
class TestMidday:
    def test_midday_calculation(self) -> None:
        date = datetime.date(2016, 7, 2)
    ...
    def test_midday_calculation_without_date_uses_today(self) -> None:
    ...
    def test_midday_in_different_timezone(self) -> None:
        aus_time = zoneinfo.ZoneInfo("Etc/GMT-10")
...
class TestLatest:
    def test_latest_calculation(self) -> None:
        date = datetime.date(2016, 7, 2)
    ...
    def test_latest_calculation_without_date_uses_today(self) -> None:
...
class TestDateTime:
    def test_datetime_creation(self) -> None:
        dt = localtime.datetime(2016, 8, 5)
        assert dt.hour == 0
        utc_dt = dt.astimezone(localtime.UTC)
        assert utc_dt.hour == 23

    def test_dst_ambiguity(self) -> None:
        # 2020-10-25T01:30 is an ambiguous dt in Europe/London as its in the
        # period when clocks go back (so it occurs twice).
...
class TestAsLocaltime:
    def test_conversion_of_gmt_dt(self) -> None:
        non_dst_datetime = datetime.datetime(
            2016, 1, 1, 23, 1, tzinfo=localtime.UTC
        )
    ...
    def test_conversion_of_bst_dt(self) -> None:
        dst_datetime = datetime.datetime(
            2016, 5, 1, 23, 1, tzinfo=localtime.UTC
        )
    ...
    def test_converts_utc_to_europe_london(self) -> None:
...
class TestQuantise:
    ...
    def test_quantise(
        ...
        if rounding_strategy:
            assert (
                localtime.quantise(dt, timedelta, rounding=rounding_strategy)
                == result
            )
        else:
            assert localtime.quantise(dt, timedelta) == result


class TestDateBoundaries:
    def test_date_boundaries_for_gmt_date(self) -> None:
    ...
    def test_date_boundaries_for_bst_date(self) -> None:
    ...
    def test_date_boundaries_for_spring_dst_boundary(self) -> None:
    ...
    def test_date_boundaries_for_autumn_dst_boundary(self) -> None:
...
class TestStartOfMonth:
    ...
    def test_start_of_month(
        self, dt: datetime.datetime, result: datetime.datetime
    ) -> None:
...
class TestAsRange:
    def test_converts_date_to_correct_values(self) -> None:
...
class TestIsInFutureOrPast:
    def test_with_aware_datetimes(self) -> None:
    ...
    def test_with_naive_datetimes(self) -> None:
...
class TestNextMidnight:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # BST -> GMT
            (
                localtime.datetime(2018, 10, 27),
                localtime.datetime(2018, 10, 28),
            ),
            (
                localtime.datetime(2018, 10, 28),
                localtime.datetime(2018, 10, 29),
            ),
            (
                localtime.datetime(2018, 10, 29),
                localtime.datetime(2018, 10, 30),
            ),
        ],
    )
    def test_dst_end_datetime(
...
class TestLatestDateForDay:
    @pytest.mark.parametrize(
        ...
        (
            (
                ...
                9,
                "2018-12-09",
            ),  # Result in last month.
            (
                "2017-01-01",
                "2018-12-08",
                9,
                "2018-11-09",
            ),  # Result in previous month.
            (
                "2017-01-01",
                "2017-03-30",
                31,
            ...
            (
                ...
                12,
                "2017-01-12",
            ),  # Result same as from date.
            (
                "2017-01-12",
                "2017-01-30",
                30,
                "2017-01-30",
            ),  # Result same as to date.
    ...
    def test_latest_date_for_day(
        ...
        end_date: str,
        day_of_month: int,
        expected_result: str | datetime.date | type[Exception] | None,
    ) -> None:
        if isinstance(expected_result, type) and issubclass(
            expected_result, Exception
        ):
            with pytest.raises(expected_result):
                localtime.latest_date_for_day(
            ...
            result = localtime.latest_date_for_day(
                start_date=factories.date(start_date),
                end_date=factories.date(end_date),
                day_of_month=day_of_month,
            )
...
class TestIsWithinTheLastYear:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                "2019-01-01",
                True,
            ),  # Just before midnight GMT
            (
                "2019-08-01 23:59:59",
                "2019-08-01",
                True,
            ),  # Just after midnight BST
    ...
    def test_returns_correct_results_for_dates(
        self,
        now_string: str,
        supplied_date_string: str,
        within_last_year: bool,
    ) -> None:
        now = factories.local.dt(now_string)
        supplied_date = factories.date(supplied_date_string)
        with time_machine.travel(now, tick=False):
            assert (
                localtime.is_within_the_last_year(supplied_date)
                == within_last_year
            )
...
class TestWithinLastWeek:
    ...
    def test_returns_correct_results_for_dates(
        self, now_str: str, supplied_date_str: str, is_within_last_year: bool
    ) -> None:
        now = factories.local.dt(now_str)
        supplied_date = factories.date(supplied_date_str)
        with time_machine.travel(now, tick=False):
            assert (
                localtime.is_within_the_last_week(supplied_date)
                == is_within_last_year
            )
...
class TestIsDST:
    ...
    def test_raises_value_error_for_naive_dt(self) -> None:
        with pytest.raises(ValueError):
...
class TestIsLocaltimeMidnight:
    ...
    def test_returns_true_for_localtime_midnights(
    ...
    def test_returns_false_for_non_localtime_midnights(
        self, dt: datetime.datetime
    ) -> None:
        assert not localtime.is_localtime_midnight(dt)

    def test_returns_false_if_timezone_differs(self) -> None:
...
class TestNextDateWithDayOfMonth:
    ...
    def test_next_date_with_day_of_month(
        self,
    ...
    def test_raises_if_datetime_is_used(self) -> None:
...
class TestConsolidateIntoIntervals:
    ...
    def test_valid_input(
        self,
        dates_passed: list[datetime.date],
        dates_expected: list[tuple[datetime.date, datetime.date]],
    ) -> None:
        assert (
            localtime.consolidate_into_intervals(dates_passed)
            == dates_expected
        )
...
class TestDatetimeFromUTCUnixTimestamp:
    @override_settings(TIME_ZONE="Europe/London")
    def test_timestamp_in_london_timezone(self) -> None:
    ...
    def test_timestamp_in_aus_timezone(self) -> None:
        timestamp = 1605804900
        dt = localtime.datetime_from_epoch_timestamp(timestamp)
        assert dt == localtime.datetime(2020, 11, 20, 2, 55)

    def test_timestamp_with_timestamp_argument(self) -> None:
        aus_time = zoneinfo.ZoneInfo("Etc/GMT-10")
        timestamp = 1605804900
    ...
    @override_settings(TIME_ZONE="Europe/London")
    def test_timestamp_british_summer_time_before_clocks_move_forward(
        self,
    ) -> None:
        """
    ...
    @override_settings(TIME_ZONE="Europe/London")
    def test_timestamp_british_summer_time_after_clocks_move_forward(
        self,
    ) -> None:
        """
        On the 29th of March 2020 at 1:00 am Europe/London = 1:00 am UTC
    ...
    @override_settings(TIME_ZONE="Europe/London")
    def test_timestamp_british_summer_time_before_clocks_move_backward(
        self,
    ) -> None:
        """
    ...
    @override_settings(TIME_ZONE="Europe/London")
    def test_timestamp_british_summer_time_after_clocks_move_backward(
        self,
    ) -> None:
        """
        On the 25th of October 2020 at 2:00 am Europe/London = 1:00 am UTC
...
class TestPeriodExceedsOneYear:
    ...
    def test_period_exceeds_one_year(
        ...
        first_dt_exceeding_one_year: datetime.datetime,
    ) -> None:
...
class TestParseDate:
    ...
    def test_errors_if_invalid(self) -> None:
...
class TestStrftime:
    ...
    def test_formats_datetime_in_local_timezone(self) -> None:
        dt = datetime.datetime(2023, 10, 1, 22, 30, 0, tzinfo=timezone.utc)  # type: ignore[attr-defined]
        fmt = "%Y-%m-%d %H:%M:%S %z"
