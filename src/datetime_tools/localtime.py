...
far_future = timezone.make_aware(
    datetime_.datetime.max - datetime_.timedelta(days=2)
)

# Timezone aware datetime in the far past.
far_past = timezone.make_aware(
    datetime_.datetime.min + datetime_.timedelta(days=2)
)
...
def as_localtime(
    ...
    Wrapper for the `django.utils.timezone` function, taking the same
    arguments.
    """
    return timezone.localtime(dt, timezone=tz)


def as_utc(dt: datetime_.datetime) -> datetime_.datetime:  # pragma: no cover
    """
...
def date(
    ...
    """
    Return the date of the given datetime in the given timezone, defaulting to
    local time.

    Note that `dt` must already be timezone-aware: it cannot be naive. It may
    be in UTC, for example, or already in current local time, in which case the
    effect will be the same as calling `.date()` on it. If `tz` is not
    provided, the function will return the date of `dt` in the local time zone.
    """
    if dt is None:
        # This is necessary as as_localtime is fine with being passed None, but
        # we don't want to accidentally do that here (since we have today as a
        # separate method).
...
def today(tz: datetime_.tzinfo | None = None) -> datetime_.date:
    """
    Return the current date in the provided timezone (or the local timezone if
    none is supplied).
    """
...
def yesterday(tz: datetime_.tzinfo | None = None) -> datetime_.date:
    """
    Return the previous date in the provided timezone (or the local timezone if
    none is supplied).
    """
...
def tomorrow(tz: datetime_.tzinfo | None = None) -> datetime_.date:
    """
    Return the next date in the provided timezone (or the local timezone if
    none is supplied).
    """
...
def days_in_the_past(
    n: int, tz: datetime_.tzinfo | None = None
) -> datetime_.date:
    """
...
def months_in_the_past(
    n: int, tz: datetime_.tzinfo | None = None
) -> datetime_.date:  # pragma: no cover
    """
    Return n months before the current date (in the provided or local
    timezone).
    """
    return today(tz) - relativedelta(months=n)


def months_in_the_future(
    n: int, tz: datetime_.tzinfo | None = None
) -> datetime_.date:  # pragma: no cover
    """
...
def date_of_day_before(
    ...
    That is, find the local date of the datetime passed in, subtract one day
    from it, and return it.

    This is frequently useful when we are converting from the upper bound of a
    datetime range, which by convention we treat as exclusive, to the upper
    bound of a date range, which by convention we consider *inclusive*.

    Note that `dt` must already be timezone-aware: it cannot be naive. It may
    be in UTC, for example, or already in current local time, in which case the
    effect will be the same as calling `.date()` on it and subtracting a day.
    If `tz` is not provided, the function will return the date before `dt` in
    the local time zone.
    """
...
) -> datetime_.datetime:  # pragma: no cover
    """
...
def midnight(
    ...
    If date is None then the current date in the passed timezone is used (if no
    timezone is supplied then we use the local timezone).
    """
    if date_or_datetime is None:
        date_: datetime_.date = today(tz)
    elif isinstance(date_or_datetime, datetime_.datetime):
        # Although this function is meant to be used on dates, we want to
        # handle datetimes properly as well, as these are frequently passed in
        # as a way of 'truncating' them to midnight on that date.
        if timezone.is_naive(date_or_datetime):
            # Default to localtime if no tz provided, as_localtime takes tz
            # aware args.
    ...
    naive_midnight = datetime_.datetime.combine(
        date_, datetime_.datetime.min.time()
    )
...
def next_midnight(
    ...
    """
    Return the datetime for midnight of the following day to the date passed
    in.
    ...
    if date_or_datetime is None:
        ...
        # Although this function is meant to be used on dates, we want to
        # handle datetimes properly as well
        if timezone.is_naive(date_or_datetime):  # pragma: no cover
            # Default to localtime if no tz provided, as_localtime takes tz
            # aware args
            date_ = date(timezone.make_aware(date_or_datetime), tz)
        else:
...
def latest(
    ...
    """
    Return a TZ-aware datetime for the latest representable datetime of the
    passed date.
    ...
    if _date is None:
        _date = today()
    naive_midnight = datetime_.datetime.combine(
        _date, datetime_.datetime.max.time()
    )
    return timezone.make_aware(naive_midnight, timezone=tz)
...
def date_boundaries(
    ...
    """
    Return a 2-tuple with the start and ending dt for the given date in the
    local timezone.

    Note, be careful of using this with __range ORM queries as such queries are
    INCLUSIVE on the boundaries.
    """
...
def month_boundaries(
    ...
    The boundary consists of a 2-tuple of: midnight of the first day of the
    month and midnight of the first day of the next month.
...
def make_aware_assuming_local(
    ...
    """
    Just a wrapper for Django's method, which will takes a naive datetime, and
    makes it timezone aware, assuming the current timezone if none is passed
    (which it isn't from this wrapper function). It will also raise an
    exception if the passed datetime is already timezone-aware.
    """
    return timezone.make_aware(dt)


def make_aware_assuming_utc(
    dt: datetime_.datetime,
) -> datetime_.datetime:  # pragma: no cover
    """
...
def quantise(
    ...
    """
    'Round' a datetime to the nearest interval given by the `timedelta`
    argument.

    For example:

        >>>> quantise(datetime.datetime(2020, 4, 1, 23, 21),
        timedelta(minutes=15)) datetime.datetime(2020, 4, 1, 23, 30)
...
def is_last_day_of_month(_date: datetime_.date) -> bool:  # pragma: no cover
    """
...
def end_of_month(dt: datetime_.datetime | None = None) -> datetime_.datetime:
    """
    Return the start datetime of the next month for dt passed - or of next
    month.
    """
    if not dt:  # pragma: no cover
        dt = now()
...
def first_day_of_month(
    dt: datetime_.datetime | None = None,
) -> datetime_.date:  # pragma: no cover
    """
...
def is_n_days_until_end_of_month(n_days: int) -> bool:  # pragma: no cover
    """
...
def is_date_within_date_range(
    date_in_question: datetime_.date,
    start: datetime_.date,
    end: datetime_.date,
) -> bool:  # pragma: no cover
    """
    Return whether a given date falls within the range of two other dates. This
    function assumes start < end.
    """
...
def is_in_the_past(dt: datetime_.datetime) -> bool:
    ...
    Note that we treat the current time as 'in the past' for the sake of this
    test. This means that any given datetime will always return True for either
    this function or `is_in_the_future`.
    """
...
def is_in_the_future(dt: datetime_.datetime) -> bool:
    ...
    """
    if not timezone.is_aware(dt):  # pragma: no cover
        raise ValueError("Datetime must be timezone-aware")
    return dt > now()


def is_future_date(_date: datetime_.date) -> bool:  # pragma: no cover
    """
...
def is_within_the_last_week(date: datetime_.date) -> bool:
    """
    Test whether a date is within a week ago of today's dates inclusive of the
    earliest date.
    """
...
def latest_date_for_day(
    ...
    """
    Given an integer day of a month, return the latest date with that day of
    the month, bounded by the supplied start_date and end_date. If no such date
    exists, return None.
    """
...
def next_date_with_day_of_month(
    ...
    """
    Given a starting `date`, return the next date with the specified
    `day_of_month`.

    If the day of the month doesn't exist in the next month, return the nearest
    date that does.
    ...
    Note: Python's datetime is a subclass of date, so the type checker will not
    prevent passing it to this function.
    """

    if isinstance(date, datetime_.datetime):
        raise TypeError("Must use a date, not a datetime.")

    if not (1 <= day_of_month <= 31):  # pragma: no cover
        raise ValueError(f"{day_of_month} is not a valid day of the month.")
    ...
    return next_date  # pragma: no cover


def date_iterator(
    start_date: datetime_.date, end_date: datetime_.date
) -> Generator[datetime_.date, None, None]:  # pragma: no cover
    """
...
def system_timezone() -> tz.tzlocal:  # pragma: no cover
    """
...
def is_dst(local_time: datetime_.datetime) -> bool:
    """
    Indicate whether the given time (and timezone is in daylight savings time
    (DST or not).
...
def is_localtime_midnight(
    ...
    """
    Return whether the supplied datetime is at midnight (in the site's local
    time zone).

    Note, the supplied datetime, which should be timezone aware, may be in any
    timezone, providing it corresponds to the moment of midnight in the site's
    local time zone.
    """
...
def is_aligned_to_midnight(
    ...
    return all(
        [
            is_localtime_midnight(range.start, tz),
            is_localtime_midnight(range.end, tz),
        ]
    )
...
def consolidate_into_intervals(
    ...
    """
    Given a sequence of dates, return tuples of (inclusive) boundaries of the
    sub-sequences where the dates are consecutive.

    If a date does not form a continuous interval with any other dates - i.e.
    it is separated by at least a day before and after the others - it forms an
    interval with itself as both boundaries.
...
def period_exceeds_one_year(
    ...
    Edge cases such as leap years and daylight savings time are handled, where
    a simple approach using only relativedelta would not be sufficient.
    """

    # We take everything as localtime, and then remove the timezone
    # information. This is to avoid false results when the period starts or
    # ends on a leap day, or when an uneven number of DST changes happen in the
    # year covered by the invoice.
    ...
    if tz_unaware_start_at.month == 2 and tz_unaware_start_at.day == 29:
        ...
        one_year_after_start_at = one_year_after_start_at.replace(
            month=3, day=1
        )
...
def parse_dt(
    value: str, tz: datetime_.tzinfo | None = None
) -> datetime_.datetime:
    """
